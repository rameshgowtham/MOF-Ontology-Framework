...
class MOFResearchAnalyzer:
    ...
    def _load_schema() -> str:
        """Loads the ontology schema from file."""
        schema_path = Path("demo2.ttl")
        try:
    ...
    @retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=4, max=10))
    def analyze_document(self, user_prompt: str, question: str, answer_type: str, custom_prompt: str = "") -> str:
        """Analyzes a document with retry logic for a specific question."""
        if not self.documents or not self.texts:
            return "-"

        context = "\n".join(text.page_content for text in self.texts)
        
        try:
            response = self.llm.invoke(
                self.unified_prompt.format(
                    context=context,
                    question=question,
                    answer_type=answer_type,
                    schema=self._schema,
                    custom_prompt=custom_prompt if custom_prompt else "Use the standard analysis approach."
                )
            )
            ...
            # More strict cleaning of the response
            if "not mentioned" in answer.lower() or not answer or answer.isspace():
                return "-"
...
def process_file(analyzer: MOFResearchAnalyzer, file: Any, questions_data: List[Dict], specific_question: Optional[str] = None) -> Optional[Dict]:
    """Processes a single file with independent analyzer instance."""
    try:
        # Create a new analyzer instance for each thread
        thread_analyzer = MOFResearchAnalyzer(analyzer.key_manager.api_keys)
        if thread_analyzer.load_pdf(file):
            results = {"File": file.name}
            
            # If a specific question is provided, filter questions
            if specific_question:
                questions_data = [q for q in questions_data if q["column_name"] == specific_question]
            
            for question_data in questions_data:
                answer = thread_analyzer.analyze_document(
                    question_data.get("question", ""), 
                    question_data["column_name"],
                    question_data.get("type", "String"),
                    question_data.get("custom_prompt", "")
                )
                column_name = question_data["column_name"]
                results[column_name] = answer
            return results
        return None
    except Exception as e:
